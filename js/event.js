function Events()
{
	...
	this.load_list = function() {
		...
		// 改为ajax请求
		$.ajax({
			url: getCafeApiUrl('events/eventList'),
			method: 'get',
			dataType: 'json',
			data: {
			...
			success: (result)=> {
				if(result.code == 200) {
					that.events = result.data.events;
	...
	this.refresh = function() {
		...
		$.ajax({
			url: getCafeApiUrl('events/eventDetail'),
			method: 'get',
			dataType: 'json',
			data: {
				pc_name: thePCInfo.pc_name,
			...
			success: (result)=> {
				if(result.code == 200) {
					for (var i=0; i<that.events.length; i++) {
						if (that.events[i].event_id == result.data.event.event_id) {
							that.events[i] = result.data.event;
					...
					that.build_event_list_html();
					that.build_event_detail_html(result.data.event.event_id);
					return;
				} else {
					sweetAlert("", translate_string(result.message), 'error');
					return;
	...
	this.join_post = function() {
		...
		$.ajax({
			url: getCafeApiUrl('events/eventJoin'),
			method: 'post',
			data: {
				pc_name: thePCInfo.pc_name,
				event_id: event_id
