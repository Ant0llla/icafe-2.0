...
function refresh_order_list()
{
	...
	path = getCafeApiUrl('memberOrders') + '?' + params.join('&');
...
function refresh_balance_history()
{
	var payload = {
	...
	path = getCafeApiUrl('memberBalanceHistory') + '?' + params.join('&');
...
function feedback_form_submit()
{
	...
	$.ajax({
		url: getCafeApiUrl('customerFeedback'),
		method: 'post',
		data: {
...
function change_password_form_submit()
{
	...
	$.ajax({
		url: getCafeApiUrl('memberChangePassword'),
		method: 'post',
		data: {
...
function ConvertToMember()
{
	...
	this.submit = function() {
		...
		$.ajax({
			url: getCafeApiUrl('convertToMember'),
			method: 'post',
			data: {
				account: account,
				birthday: birthday,
