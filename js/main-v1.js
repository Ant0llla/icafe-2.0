	...
	$.ajax({
		...
		success: (result) => {
			if(result.code!=200){
				...
				$('#loginForm input[name=username]').prop('disabled', false);
				$('#loginForm input[name=password]').prop('disabled', false);
				return;
			}
...
function refresh_order_list()
{
	...
	// const cafeId = clientMemberInfo?.license?.object_id ?? "";
	let path = "https://" + theSettings.license_info.license_server_code + ".icafecloud.com/api/v2/cafe/" + theCafe.id + "/memberOrders";
	...
	$.ajax({
		...
		dataType: 'json',
		headers: {
			'Authorization': 'Bearer ' + clientMemberInfo?.token
		},
		success: (result) => {
...
function OnCommand(strCmd, strParam)
{
	...
	if (strCmd == "APIResponse") {
		...
		if (api_action.indexOf('type=sms_code') >= 0) {
			var timer = $('#sms_timer_code');
			if (data.code == 200) {
				SmsClassFun.smsCountDown();
				return;
			} else {
...
function TopupLogin() {
	...
	this.show_topup_login = function () {
		...
		$('.myModalTopupLogin input[name=topup_memeber_email]').val('');
		$('.myModalTopupLogin input[name=promo_code]').val('');
		$('.myModalTopupLogin input[name=topup_login_amount]').val(theSettings.mini_qr_payment);
	...
	this.topupTimeCalculate = function () {
		...
		topupTimeCalculateTimer = setTimeout(() => {
			...
			if(reqData['member_account'] == '')
				return;
			$.ajax({
				url: "https://" + theSettings.license_info.license_server_code + ".icafecloud.com/api/v2/cafe/"+theCafe.id+"/calculate/amount",
				method: "post",
				data: reqData,
				dataType: "json",
				success: (data) => {
	...
	this.topupAmountCalculate = function () {
		...
		topupAmountCalculateTimer = setTimeout(() => {
			...
			if(reqData['member_account'] == '')
				return;
			$.ajax({
				url: "https://" + theSettings.license_info.license_server_code + ".icafecloud.com/api/v2/cafe/"+theCafe.id+"/calculate/time",
				method: "post",
				data: reqData,
				dataType: "json",
				success: (data) => {
	...
	this.topup = function (reqData) {
		...
		$.ajax({
			url: "https://" + theSettings.license_info.license_server_code + ".icafecloud.com/api/v2/cafe/" + theCafe.id + "/getTopupUrl",
			method: "post",
			data: reqData,
			dataType: "json",
			success: (data) => {
				...
				if(data.result == 1)
				{
					...
					if(data.type == 'razorpay'){
						$('.myModalTopupLogin #login_payment_qr').remove()
		...
		$.ajax({
			...
			success: (data) => {
				...
				if(data.result == 1)
				{
					...
					if(data.type == 'razorpay'){
						$('.myModalTopup #payment_qr').remove();
...
function Events()
{
	...
	this.gamecode2names = [
		...
		{ code: 'apex', name: 'Apex Legends' },
		{ code: 'all', name: 'Fortnite, Dota 2, Valorant, LOL, CSGO, Apex' },
	];
...
function Rank()
{
	this.show = function() {
		...
		let gameCode = ['fortnite', 'lol', 'dota2', 'valorant', 'csgo', 'apex'];
...
const SmsClassFun = {
	...
	sendCode: function () {
		...
		if (smsIcafeId) {
			CallFunction("API type=sms_code&icafe_id=" + encodeURIComponent(smsIcafeId) + "&sms_recipient=" + encodeURIComponent(phone));
		} else {
			timer.prop('disabled', false);
			sweetAlert('', translate_string('Fail in send!'), 'error');
		}
	},
	// 验证短信验证码是否匹配
	verifySmscode: function (callback) {
		var phone = $("#form-register-member input[name=phone]").val();
		var code = $("#form-register-member input[name=code]").val();
		$.ajax({
			url: "https://" + theSettings.license_info.license_server_code + ".icafecloud.com/api/clientapi.php",
			method: "post",
			data: { type: 'check_sms_code', sms_recipient: phone, sms_code_value: code },
			dataType: "json",
			success: (data) => {
				if (data.code == 200) {
					callback(data);
