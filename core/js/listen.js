	...
	ICAFEMENU_CORE.onCmd("CallExeDone", strParam => {
		...
		for (var i = 0; i < ids.length; i++) {
			...
			theGames.forEach(function (obj) {
				if (obj.pkg_id == ids[i]) {
					html += ('<a class="run-game-icon" href="javascript:rungame_show_dialog(' + obj.pkg_id + ')" data-toggle="tooltip" data-placement="bottom" title="' + obj.pkg_name + '"><img src="' +  ICAFEMENU_CORE.icons_path(obj.pkg_id + '.png')+'" onerror="this.src=\'images/default-icon.png\'"></a>');
				}
			});
