...
const ICAFEMENU_CORE = {
	 ...
	 */
	root_path(path){
		// /html/themes/icafemenu/ => /html/
		let root = this._main_path.replace(/[^\/]+\/[^\/]+\/$/,'')
	 ...
	 */
	main_path(path){
		return this._main_path + (path||"").replace(/^\//,'');
	},
	write_script(path){
...
// /html/themes/icafemenu/main.htm => /html/themes/icafemenu/
